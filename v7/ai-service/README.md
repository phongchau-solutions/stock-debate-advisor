...
# Demo/Streamlit
pip install -r deps/requirements-demo.txt
```
...
Or via uvicorn:
```bash
uvicorn src.handlers.main:app --reload --port 8000
```

### 4. Run Demo UI (separate terminal)
```bash
streamlit run src/handlers/demo.py
```
...
ai-service/
├── src/           # Source code
│   ├── core/      # Engine, config, constants
│   ├── handlers/  # Lambda, API, demo entries
│   └── utils/     # Models and utilities
├── deps/          # Environment-specific requirements
└── scripts/       # Shell scripts (symlinked to /scripts)
```
